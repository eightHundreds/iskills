...
- 告警 Modal 提供「复制给 Agent」：把当前冲突的修复说明写入剪贴板，方便交给其他 Agent 处理。
- 收藏夹详情「复制路径」旁增加「解绑来源」：去掉 Git 来源后不再检测该仓库更新，技能仍留在收藏夹。

### 变更

- 窄屏全屏详情的关联位置改为只显示数量，按 `→` 进入后再列出全部原始/使用/依赖路径。
- 收藏夹全屏详情展示技能的本地位置（与「复制路径」相同）；该字段可焦点，Enter 或点击复制路径。

### 修复

- 窄屏全屏详情可用 ↑/↓ 在标签、备注、GitHub 来源之间移动焦点，Enter 激活当前字段。
...
- 主浏览器详情列底部增加「复制路径」，把当前技能的本地位置写入系统剪贴板。

### 修复

- 收藏夹更新 MCP 时，Claude / Grok 只写停用层，不再把 `disabled` / `enabled` 写回服务器对象。

## [0.2.56] - 2026-08-28

### 新增

- 报错时把本次运行时间线写入收藏夹 `.local/logs/`（npm 风格），并提示完整日志路径。成功、取消、Ctrl+C 不写文件。

### 修复

- 搜索收藏和从 Git / 本地根导入时，仓库内同名技能仍用原名列出，仅同名项旁边标注仓库路径；一次只能收藏其中一份。

## [0.2.55] - 2026-08-23

### 变更

- MCP Agent 读写拆成 dialect 适配器；安装位置评审技能与 MCP 共用同一套步骤机。领域只返回可写路径，UI 再画标签。
