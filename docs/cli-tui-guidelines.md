...
- 浏览器同时显示上下文操作和导航快捷键时，上下文操作行 MUST 紧邻并位于导航行上方，不得插入空行；`m 更多操作`位于上下文操作行首位。导航行按查看、删除、搜索、快捷键、退出排序。
- 网络或 Git 操作一旦产生可感知等待，SHOULD 及时显示当前阶段；可恢复失败 SHOULD 保留上下文并允许重试。

### COPY-1 技能术语

- 所有 TUI 对同一 Skill 字段 MUST 使用 [CONTEXT.md](../CONTEXT.md) 定义的规范术语；候选技能与收藏技能的详情不得为同一字段使用同义标签。
- Skill 的文本说明字段 MUST 显示为“描述”。“完整描述”不是独立字段或显示标签。

### PROMPT-3 收藏夹添加方式

- 从收藏夹浏览器添加到当前项目或全局 Agent 目录时，MUST 在写入前使用同一个带 Tab 的安装配置界面完成安装位置、软链或复制方式和目标目录的选择；不得依次弹出多个独立选择器。软链为默认方式，单次批量添加的所有技能和目标使用同一方式。
- 安装配置界面 MUST 包含“安装位置”“添加方式”“目标目录”和“确认”Tab；确认 Tab MUST 展示本次选择，确认后才开始写入。
...
- 浏览器内快捷键和更多操作浮层 MUST 在当前 Tab 的完整列表 viewport 内水平、垂直居中；不得只相对当前可见的技能行居中。
...
- 同一流程在列表与详情间切换时，标题、Tab、内容框和 footer 的垂直锚点 MUST 保持稳定；详情内容超出固定框体时 MUST 在框内滚动，不得以扩展页面高度呈现。
- 文本编辑 MUST 不损坏 Unicode 输入；若暴露 Delete、Home、End 等键，其行为应符合常见终端输入习惯。

### VISUAL-1 终端色彩语义

- TUI 的颜色 MUST 通过 `termcnColors` 使用；新增视图不得直接写入 hex 值或自行定义第二套色板。普通 CLI、`--json` 和非 TTY 路径不得输出颜色。

| Token | 值 | 语义 | 允许使用 |
| --- | --- | --- | --- |
| `foreground` | `#FFFFFF` | 主要可读内容 | 标题、名称、正文 |
| `muted` | `#9CA3AF` | 次要或已知状态 | 描述、路径、计数、帮助、同源收藏 `★` |
| `border` | `#4B5563` | 静态结构边界 | 普通面板、列表、详情和 Tab 内容边框 |
| `primary` | `#7C3AED` | 当前可操作对象 | 光标、激活 Tab、选中控件、正在编辑的输入框边框、模态层边界 |
| `error` | `#EF4444` | 失败、危险或已确认冲突 | 错误消息、破坏性动作、异源同名 `☆` |

- 普通容器边框 MUST 使用 `border`，不得因列表获得焦点而改为 `primary`；焦点必须由 `›`、反白、加粗或当前行的 `primary` 文本表达。正在编辑的单行输入框和遮挡背景输入的模态层 MAY 使用 `primary` 边框。
- `★` 表示同一远端来源已收藏；`☆` 表示收藏夹存在同名技能但来源不同或尚未验证。来源已确认不同的 `☆` MUST 使用 `error`，并在详情或确认中显示双方的仓库和来源路径。
- `›`、`●`/`○`、`★`/`☆`、`!` 等字符是状态的主载体；颜色只能加强语义，不能成为当前项、选择、默认确认、错误或冲突的唯一辨识方式。
...
- 远端技能是否同源，由规范化 Git 仓库来源和仓库内 Skill 路径共同确定，不能只比较显示名称。GitHub search 来源按小写 `owner/repo` 归一；路径使用无 `.`、`..` 的 POSIX 相对路径。其他来源的归一规则见数据完整性契约。
- 在执行动作前 MUST 区分：未收藏、同源已收藏、异源同名冲突。
- 未收藏项的 Enter 开始校验并收藏；同源已收藏项不可重复收藏，并提示改用 update；异源同名冲突的 Enter 在 staging/校验后进入替换确认。确认提示 MUST 展示双方的仓库和来源路径；仓库相同时至少显示各自的仓库内路径。
