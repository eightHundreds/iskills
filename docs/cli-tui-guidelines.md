...
### BROWSER-2 更多操作与引用转副本

- “当前项目”Tab MUST 将顶层路径为软链的技能标记为 `引用 · <技能名>`，不以是否来自收藏夹判断引用类型。
- `m` MUST 作为低频操作的“更多操作”入口。存在已选技能时作用于全部已选技能，否则作用于当前技能；只有全部作用对象都是可解析的技能目录软链时，footer 才显示 `m 更多操作` 并响应该按键。混合选择不得静默跳过普通目录技能。
- 更多操作 MUST 使用浏览器内浮层并停用背景输入。浮层 MUST 展示当前技能或已选技能数量，在范围摘要与操作项之间保留一行空白；`↑/↓` 移动、Enter 执行、Esc 返回。
- “将引用转为副本” MUST 只在“当前项目”Tab 提供，不得扩展到“全局”或“收藏夹”Tab。该操作是浏览器便捷文件操作，不属于 CLI-3 的核心状态变更，无需提供非交互入口。
- 引用转副本 MUST 原地把顶层软链替换为普通目录，且不得修改软链最终指向的来源目录。用户已在更多操作中明确选择该动作，不得追加二次确认。
- 技能内部软链最终指向技能根目录内时，MUST 物化为副本内容；无法解析、形成循环或最终指向技能根目录外时，MUST 在提交前拒绝整批转换。
- 批量转换 MUST 全有或全无：所有副本完成暂存和校验后才能替换原软链；路径替换或链接状态写入失败以及 Ctrl+C 中断时，MUST 恢复全部原软链和原状态。
- 转换期间 MUST 停用其他浏览器动作，并显示当前项和总进度。成功后 MUST 保留 Tab、Agent、搜索和光标，清空选择并刷新列表；失败后 MUST 保留选择和光标，并显示具体失败项与原因。
- 转换 `state.links` 中的 `usage` 或 `dependent` 位置时 MUST 删除该路径记录；转换 `origin` 时还 MUST 删除同一技能的 `dependent` 记录。其他仍直接指向收藏夹的 `usage` MUST 保留，未受收藏夹管理的本地引用不得创建链接状态、metadata、baseline 或 Git 记录。
